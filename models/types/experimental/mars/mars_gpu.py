...
class MARSGPU(BaseEstimator, RegressorMixin):
    """
    ...
    def _forward_pass(self, X, y):
        ...
        r = cp.asarray(y.copy())  # Residuals of the model

        # Iterate until the max terms is reached or no further improvements
        while len(self.basis_) < self.max_terms:

            # Stores occurrences of the best results
            ...
            for index in range(n_features):
                ...
                for knot in knot_splits:
                    for direction in [1, -1]:
                        ...
                        self.coef_ = cp.linalg.lstsq(
                            basis_matrix, r - self.intercept_, rcond=None
                        )[0]

                        mse = mean_squared_error(r.get(), r.get()) \
    ...
    def _backward_pass(self, X, y):
        ...
        while n_basis_functions > 0:
            ...
            for idx in range(n_basis_functions):
                ...
                updated_basis_matrix = cp.delete(basis_matrix, idx, axis=1)

                # Fit the coefficients for the updated basis matrix
                updated_coef_ = cp.linalg.lstsq(
            ...
            basis_matrix = cp.delete(basis_matrix, best_removed_index, axis=1)
            gcv_score = best_gcv
            n_basis_functions -= 1
    ...
    def _generate_candidate_splits(self, x_column):
        ...
        unique_values = cp.unique(x_column)
    ...
    def _evaluate_single_basis(self, X, basis_function):
        ...
        if direction == 1:
            return cp.maximum(0, x_col - knot)
        elif direction == -1:
            return cp.maximum(0, knot - x_col)
    ...
    def _evaluate_basis(self, X):
        ...
        basis_matrix = cp.zeros((n_samples, n_basis))
    ...
    def permutation_importance(self, model, X, y, metric):
        ...
        for col in X.columns:
            X_shuffled = X.copy()
            X_shuffled[col] = cp.random.permutation(X_shuffled[col])
