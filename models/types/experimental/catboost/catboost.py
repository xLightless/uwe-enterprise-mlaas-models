...
# Enhanced CatBoost Regressor with automatic feature selection, handling of outliers, cross-validated model building, and advanced optimisation techniques.
class CatBoost(BaseEstimator, RegressorMixin):
    ...
    # Creates a CatBoostRegressor instance with the current parameters, handling compatibility between bootstrap_type and subsample.
    ...
    # Processes outliers in the training data using either clipping or removal based on robust scaling and configurable thresholds.
    ...
    # Performs feature selection using specified method (mutual information or F-regression) to identify the most informative features for the model.
    ...
    # Implements k-fold cross-validation to create an ensemble of models trained on different data subsets for more robust predictions.
    ...
    # Uses Bayesian optimisation to find optimal hyperparameters for the model, evaluating candidates using customised validation.
    ...
    # Implements a step-decay learning rate schedule to reduce learning rate as training progresses for better convergence.
    ...
    # Executes the complete model training pipeline: outlier handling, feature selection, hyperparameter optimisation, cross-validation, and SHAP value calculation.
    def fit(self, X, y, **fit_params):
        ...
        # IMPORTANT: Removed passing feature_names to fit method - it must be set during model creation
    ...
    # Makes predictions using either the ensemble of cross-validated models (if available) or the single trained model, applying feature selection as needed.
    def predict(self, X):
        X = np.asarray(X)
    ...
    # Saves both the complete wrapper class and the inner CatBoost model to separate files for flexibility in deployment.
    def save_model(self):
    ...
    # Loads a trained model from file, supporting both the complete wrapper class and standalone CatBoost models.
    @classmethod
    ...
    # Visualises feature importance using SHAP values with different plot types to understand feature impact on predictions.
    ...
    # Creates dependence plots showing how a feature's effect on predictions varies across its value range, optionally with feature interactions.
    ...
    # Generates partial dependence plots showing marginal effects of features on predicted outcomes, independent of other features.
    ...
    # Visualises training and validation metrics over iterations to evaluate convergence and potential overfitting.
